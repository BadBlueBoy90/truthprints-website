...
/* ---------- NT-Galerie: Apg–Offb aus den breiten Kapitel-Gemaelden ----------
   Tafeln kommen lazy aus discover-data.js (2 MB — erst beim Klick auf den Tab
   geladen). Zufaellige Auswahl aus allen Nicht-Evangelien-Buechern. */
BOOKS.nt = {
  key:'nt',
  short:{ en:'Acts – Revelation', de:'Apg – Offenbarung' },
  name:{ en:'Acts to Revelation', de:'Apostelgeschichte bis Offenbarung' },
  countLabel:{ en:'1035 Plates', de:'1035 Tafeln' },
  galleryLabel:{ en:'One thousand thirty-five plates — a selection', de:'Tausendfünfunddreißig Tafeln — eine Auswahl' },
  galleryTitleEn:'Acts to Revelation, <em>plate by plate.</em>',
  galleryTitleDe:'Apostelgeschichte bis Offenbarung, <em>Tafel für Tafel.</em>',
  plates: [],
};
const NT_BOOK_NAMES = {
  acts:['Acts','Apostelgeschichte'],romans:['Romans','Römer'],
  '1corinthians':['1 Corinthians','1. Korinther'],'2corinthians':['2 Corinthians','2. Korinther'],
  galatians:['Galatians','Galater'],ephesians:['Ephesians','Epheser'],philippians:['Philippians','Philipper'],
  colossians:['Colossians','Kolosser'],'1thessalonians':['1 Thessalonians','1. Thessalonicher'],
  '2thessalonians':['2 Thessalonians','2. Thessalonicher'],'1timothy':['1 Timothy','1. Timotheus'],
  '2timothy':['2 Timothy','2. Timotheus'],titus:['Titus','Titus'],philemon:['Philemon','Philemon'],
  hebrews:['Hebrews','Hebräer'],james:['James','Jakobus'],'1peter':['1 Peter','1. Petrus'],
  '2peter':['2 Peter','2. Petrus'],'1john':['1 John','1. Johannes'],'2john':['2 John','2. Johannes'],
  '3john':['3 John','3. Johannes'],jude:['Jude','Judas'],revelation:['Revelation','Offenbarung'],
};
let _ntLoading=false;
function ensureNtGallery(cb){
  if(BOOKS.nt.plates.length){cb();return;}
  if(_ntLoading)return;
  _ntLoading=true;
  const s=document.createElement('script');
  s.src='discover-data.js';
  s.onload=()=>{
    const all=(window.TP_DISCOVER||[]).filter(p=>NT_BOOK_NAMES[p.book]);
    // Zufallsauswahl: 24 Tafeln quer durch Apg–Offb, jede Session neu
    const pick=[];
    const pool=all.slice();
    while(pick.length<24&&pool.length){pick.push(pool.splice(Math.floor(Math.random()*pool.length),1)[0]);}
    BOOKS.nt.plates=pick.map(p=>({
      img:p.img, refEn:p.ref_en, refDe:p.ref_de,
      en:(NT_BOOK_NAMES[p.book]||[p.book])[0], de:(NT_BOOK_NAMES[p.book]||[p.book,p.book])[1],
    }));
    _ntLoading=false; cb();
  };
  s.onerror=()=>{_ntLoading=false;};
...
if (cur && window.matchMedia('(hover:hover)').matches){
  ...
  /* Exact resting point on the Lord's head, via background cover-math.
     hero-dawn.jpg is 2560×1428; .hero-bg uses background-size:cover with
     background-position:center 35%. His head sits at ~63% / 16% of the image. */
  function headPoint(){
    ...
    return { x: ox + 0.63*dw, y: oy + 0.165*dh };
  }
...
(function(){
  ...
  if(!hero||!c) return;
  // rest the spotlight on the Lord's head (≈63% / 16.5%) so it agrees with the glow
  let tx=0.63,ty=0.165,ax=0.63,ay=0.165, flick=0;
  const ctx=c.getContext('2d');
  /* halbe interne Aufloesung — beim weichen Spotlight unsichtbar, 4x weniger Fill-Arbeit */
  function resize(){c.width=Math.round(hero.offsetWidth*0.5);c.height=Math.round(hero.offsetHeight*0.5);}
  ...
  hero.addEventListener('mouseleave',()=>{tx=0.63;ty=0.165;});
  /* Motion pro Frame (Lerp/Flicker) auf 30 fps ausgelegt -> Optik bleibt gleich */
  function paint(){
    ax+=(tx-ax)*0.11; ay+=(ty-ay)*0.11; flick+=0.16;
  ...
  /* Der Fackel-Canvas malte 60x/s den GANZEN Bildschirm — auch weit unterhalb des Heros.
     Jetzt: nur wenn der Hero sichtbar ist, Tab aktiv ist, und gedrosselt auf ~30 fps. */
  let visible=true, running=false, raf=0, last=0; const MININT=1000/30;
  function frame(now){
    if(!visible||document.hidden){running=false;raf=0;return;}
    raf=requestAnimationFrame(frame);
    if(now-last<MININT) return; last=now;
    if(document.body.classList.contains('motion-off')){ctx.clearRect(0,0,c.width,c.height);ctx.fillStyle='rgba(10,7,5,0.55)';ctx.fillRect(0,0,c.width,c.height);return;}
    paint();
  }
  function start(){ if(running)return; running=true; last=0; raf=requestAnimationFrame(frame); }
  function stop(){ running=false; if(raf)cancelAnimationFrame(raf); raf=0; }
  if('IntersectionObserver' in window){
    new IntersectionObserver(es=>es.forEach(e=>{ visible=e.isIntersecting; visible?start():stop(); }),{threshold:0.01}).observe(hero);
  }
...
(function(){
  ...
  const ids=['why','book','shop','story','plates','wallart','editions','about','newsletter'];
...
/* ---------- CINEMATIC VERSE PARALLAX (all .jbreak-img) ---------- */
(function(){
  const imgs=Array.from(document.querySelectorAll('.jbreak-img'));
...
(function(){
  ...
  // Vers-des-Tages: unscharfes Tagesgemaelde driftet beim Scrollen (Basis-scale bleibt)
  const vb=document.querySelector('.votd-bg');
  if(vb)items.push({el:vb,sp:0.14,scale:1.12});
  // Publisher (Lia): the ornamental pattern drifts gently behind her portrait
  const ap=document.querySelector('.ab-pattern');
  if(ap)items.push({el:ap,sp:0.09});
  ...
  function update(){
    ...
    for(const it of items){
      ...
      it.el.style.transform=`translate3d(0,${(off*it.sp).toFixed(1)}px,0)`+(it.scale?` scale(${it.scale})`:'');
    }
  }
...
document.querySelectorAll('[data-galbook]').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  if(state.galBook==='nt'&&!BOOKS.nt.plates.length){ensureNtGallery(buildGallery);return;}
  buildGallery();
}));
...
(function(){
  ...
  if(bar){
    ...
    const hideZones=['editions','newsletter'].map(id=>document.getElementById(id)).filter(Boolean);
